...
group_name="04-Sep/UnitTests-Dataset-Shuffled-Split"
...
temporal_correlation_graph=False # Whether to compute a temporal correlation graph and combine it with the existing graph.
...
aggr_list_diffusion="mean, max" # "add,mean,max" # List of aggregation methods to use (e.g., "add,mean,max"). If None, the default aggregation of the convolution layer is used.
