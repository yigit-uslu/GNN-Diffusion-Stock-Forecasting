...
CUDA_VISIBLE_DEVICES=1 # Set this to the GPU you want to use, e.g., 0, 1, 2, etc.
...
cond_tconv_kernel_type="gated-causal-conv1d"
cond_tconv_kernel_size=7 # this value will be overwritten in the code to ensure causality and proper downsampling
tconv_kernel_type="residual-gated-causal-conv1d" # "mlp" # "conv1d" # "residual-causal-conv1d" # "residual-gated-causal-conv1d"
tconv_kernel_size=3
group_name="23-Sep/GUNet-Sweeps-Tp-$past_window-Th-$future_window-tconv_kernel_type-${tconv_kernel_type}"
...
dataset_split_strategy="chronological" # "random"
train_dataset_fraction=0.8 # 0.9
corr_threshold=0.6 # 0.7 Thresholding for the correlation matrix
sector_bonus=0.05 # Bonus for stocks sharing the same sector
past_window=$past_window # 25 Past window of features including closing prices and daily log returns
future_window=$future_window # 10 Set these windows equal =$past_window
...
###############################
lr_diffusion=2e-4 # 2e-3 <-> 1e-2
weight_decay_diffusion=1e-6 # even 1e-4 is maybe too much. 
lr_sched_gamma_diffusion=0.99 # 0.5 # 0.9
grad_clipping_constant_diffusion=1.0 # 5.0
...
dropout_rate_diffusion=0.0 # dropout did not work well in previous experiments
###############################
hidden_dim_diffusion=64 # 
###############################
x_batch_size_diffusion=50 # 
diffuse_n_samples_diffusion=500 # 500
sampler_diffusion="ddpm"
batch_size_diffusion=1 # 10
edge_features_nb=2 # 2
pool_ratio_diffusion=1.0 # 0.5
n_convs_per_block_diffusion=2
pool_layer_diffusion="max" # max worked better than mean in previous experiments
...
for batch_size_diffusion in "${batch_size_diffusion_values[@]}"; do
    ...
    for norm_layer_diffusion in "${norm_layer_diffusion_values[@]}"; do
        ...
        CUDA_VISIBLE_DEVICES=$CUDA_VISIBLE_DEVICES accelerate launch --config_file ./accelerate_configs/config.yaml --num_processes 1 ./main_${SCRIPT}_diffusion_learner.py \
                                                                  ...
                                                                  --dataset_name="${dataset_name}" --data_dir="${data_dir}" --dataset_split_strategy="${dataset_split_strategy}" \
                                                                  ...
                                                                  --pool_layer_diffusion="${pool_layer_diffusion}" \
                                                                  --dropout_rate_diffusion=$dropout_rate_diffusion  --pool_ratio_diffusion=$pool_ratio_diffusion --apply_gcn_norm_diffusion=$True \
                                                                  --tconv_kernel_type="${tconv_kernel_type}" --tconv_kernel_size=$tconv_kernel_size \
                                                                  --cond_tconv_kernel_type="${cond_tconv_kernel_type}" --cond_tconv_kernel_size=$cond_tconv_kernel_size &
